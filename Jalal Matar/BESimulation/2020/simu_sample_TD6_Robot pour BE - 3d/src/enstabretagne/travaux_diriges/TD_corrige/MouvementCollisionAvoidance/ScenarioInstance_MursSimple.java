...
public class ScenarioInstance_MursSimple implements IScenarioInstance {
	...
	public IScenario getScenarioInstance(long seed) {
		...
		List<Point3D> lP;
		// essyer de fair un nouveau murs
		WallFeatures wfex = new WallFeatures("RF", Color.ORANGE, 1, 0.2, 2.5);
		WallInit win;
		lP = new ArrayList<>();
		...
		lP.add(new Point3D(0, 0, 0));
		win = new WallInit("Mur Simple", new Point3D(0, 0, 0), Point3D.ZERO, lP);
		mcaf.getWalls().put(win, wfex);
		...
		WallFeatures wf = new WallFeatures("RF", Color.BROWN, 1, 0.2, 1.5);
		WallInit wi;
		lP = new ArrayList<>();
		lP.add(new Point3D(0, 0, 0));
		lP.add(new Point3D(10, 0, 0));
		lP.add(new Point3D(10, 10, 0));
		lP.add(new Point3D(0, 10, 0));
		wi = new WallInit("Mur Simple", new Point3D(5, 10, 0), Point3D.ZERO, lP);
		mcaf.getWalls().put(wi, wf);

		// murs commme un escalier 
		wtargetF = new WallFeatures("RF", Color.BROWN, 1, 0.2, 1.5);
		lT = new ArrayList<>();
		lT.add(new Point3D(0, 0, 0));
		lT.add(new Point3D(0, 5, 0));
		lT.add(new Point3D(-5, 5, 0));
		lT.add(new Point3D(-5, 10, 0));
		lT.add(new Point3D(-10, 10, 0));
		
		wtargetI = new WallInit("Mur Simple", new Point3D(39, 10, 0), Point3D.ZERO, lT);
...
//		List<Point3D> lT;
		...
		RobotFeatures rf = new RobotFeatures("RF", 10, 100.0, 1);
		RobotInit ri = new RobotInit("RI", Color.AQUA, new Point3D(45, 5, 1), new Point3D(0, 0, 45), false,
				new Point3D(3, 4, 0));
		mcaf.getRobots().put(ri, rf);

		// Création du robot méchant
		RobotFeatures rfbad = new RobotFeatures("En", 0, 0, 1);

		RobotInit rb = new RobotInit("BadGuy", Color.MEDIUMVIOLETRED, new Point3D(10, 15, 0), new Point3D(0, 0, 90),
				true, Point3D.ZERO);
